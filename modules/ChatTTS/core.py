...
class Chat:
    ...
    def load_models(self, source='huggingface', force_redownload=False, local_path='<LOCAL_PATH>', **kwargs):
        ...
        self._load(**{k: os.path.join(download_path, v) for k, v in
                      OmegaConf.load(os.path.join(download_path, 'config', 'path.yaml')).items()}, **kwargs)
    ...
    def _load(
        ...
        if not device:
            ...
            # self.logger.log(logging.INFO, f'{device}模式')

        if vocos_config_path:
            vocos = Vocos.from_hparams(vocos_config_path).to(device).eval()
            assert vocos_ckpt_path, 'vocos_ckpt_path should not be None'
            vocos.load_state_dict(torch.load(vocos_ckpt_path, weights_only=False))
        ...
        if dvae_config_path:
            ...
            dvae.load_state_dict(torch.load(dvae_ckpt_path, map_location='cpu', weights_only=False))
        ...
        if gpt_config_path:
            ...
            gpt.load_state_dict(torch.load(gpt_ckpt_path, map_location='cpu', weights_only=False))
            if compile and 'cuda' in str(device):
                gpt.gpt.forward = torch.compile(gpt.gpt.forward, backend='inductor', dynamic=True)
            self.pretrain_models['gpt'] = gpt
            spk_stat_path = os.path.join(os.path.dirname(gpt_ckpt_path), 'spk_stat.pt')
            assert os.path.exists(spk_stat_path), f'Missing spk_stat.pt: {spk_stat_path}'
            self.pretrain_models['spk_stat'] = torch.load(spk_stat_path, weights_only=False).to(device)
        ...
        if decoder_config_path:
            ...
            decoder.load_state_dict(torch.load(decoder_ckpt_path, map_location='cpu', weights_only=False))
